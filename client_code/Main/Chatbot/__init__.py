...
class Chatbot(ChatbotTemplate):
  ...
  def draw_filelink_id(self, pid):
    if len(pid):
      down_url = f"https://deep-chief-reality.anvil.app/_/api/download_id/{pid}".replace("_", "\_").replace(" ", "%20")
  ...
  def draw_filelink_name(self, pfile, prt, pname):
    if len(pname) and "source" not in pfile:
      ...
      down_url = f"https://deep-chief-reality.anvil.app/_/api/download_name/{pfile}".replace("_", "\_").replace(" ", "%20")
      if "." in pname: pname = pfile
  ...
  def timer_1_tick(self, **event_args):
      ...
      if hasattr(self, 'task') and self.task:
          ...
          if progress:
              self.l_status.text = progress.get("progress", "Waiting...")  # Update UI with progress
          else:
          ...
          if self.task.is_completed():  # Stop polling when done
              ...
              if "```json" in reply:
                ...
                if isinstance(xtra, dict) and ("file_ids" in xtra.keys() or "file_id" in xtra.keys()):
                ...
                elif str(xtra.get("confidence", "")).upper() == "MEDIUM" or str(xtra.get("confidence", "_"))[0].isdigit():
                  self.l_confidence.text = "DOUBTS"
                  self.l_confidence.background = "amber"
                elif xtra.get("confidence"):
                  self.rt_reply.content += self.remove_markdown(history + "SPECULATIVE ANSWER. TRY INCREASING CONTEXT LENGTH OR PROVIDING MORE BACKGROUND\n")
              ...
              if "#Special#" in reply:
                ...
                if "#Scorefile#" in reply:
                  self.rt_reply.content += self.remove_markdown(history + rep_parts[4] + "\nScorecard ready for downloading \n")
                  ...
                  f_names, rep_text = self.extract_and_remove(re.sub(r'\[([^\]]+)\]\([^\)]+\)', '', rep_parts[4]))  
